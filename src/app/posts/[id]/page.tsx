...
const getPostById = async(postID: string) => {
    const response = await fetch(`http://localhost:3000/api/posts/${postID}`, {
        ...
        next: {               //TODO: This is CLEAR the cache of the server component AND  then it will fetch the new data
            revalidate: 5000, //TODO: The revalidate property is specific to Next.js's Incremental Static Regeneration (ISR)/ Dynamic Server Component now feature
        }
    })
...
const POSTID = async ({ searchParams, params }: any) => {
    const RESS = await getPostById(params.id);
